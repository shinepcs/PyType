...
- Daily Training: 로컬 날짜와 콘텐츠 버전으로 결정되는 30문제 무제한시간 비랭크 훈련
- Practice: 원하는 문법을 골라 시간 제한 없이 연습하는 비랭크 모드
- Sample Logic: 실제 실행 가능한 2~3줄 Python 로직만 연습하는 Practice 계열 비랭크 모드
- Beginner Guide: 한국어 설명이 포함된 실무형 짧은 함수·미니 프로그램 50개를 장문으로 타이핑하는 비랭크 모드

Beginner Guide는 세로 모드로 시작하며, 가로/세로 버튼으로 원문·입력 영역을 좌우 또는 상하로 바꿀 수 있습니다. 세로 모드는 양쪽을 3줄 높이로 표시하고 현재 입력 줄을 가운데에 두도록 함께 스크롤합니다. 선택한 레이아웃은 이 브라우저에 저장됩니다. 가로 모드에서도 원문과 입력의 첫 줄은 같은 높이에서 시작합니다.

오타와 목표 시간을 넘긴 문제는 같은 세션에서 3~7문제 뒤 다시 등장합니다. 정확도, 분당 타수(타/분), 실시간 초당 타수, 콤보, 점수, 위험도, 문법별 숙련도는 `PRD.md`의 공식으로 계산됩니다. 각 모드의 결과 화면에서는 같은 모드·같은 Practice 설정의 직전 기록과 최근 5회 평균을 비교해 점수, 정확도, 분당 타수, 해결 수, 문제당 평균 시간의 변화를 보여줍니다. 제한시간은 온라인 랭크 대상인 Quick Play에만 적용됩니다.

Level 2에서는 미입력 정답 문자를 화면에 미리 표시하지 않습니다. 답안을 입력한 뒤 `Enter`로 제출하면 정답·오답을 표시하고, 아무 키나 누르면 다음 문제로 넘어갑니다. Quick Play에서는 문제를 푸는 동안 세션 시간과 위험도가 멈추며, 정답이면 3초 보너스, 첫 오타에는 한 번만 2초 패널티가 적용됩니다. 이 규칙 이후 Quick 랭킹은 `1.0.0-r2` 그룹으로 분리됩니다.

모든 PC 게임 화면의 전투 레인은 주변 Quick 랭킹과 현재 접속자의 최고 점수를 합쳐 현재 점수보다 낮은 경쟁자는 왼쪽, 높은 경쟁자는 오른쪽에 표시합니다. 경쟁자의 점수를 넘으면 해당 닉네임과 함께 추월 효과가 나타납니다. 오른쪽 PLAYING NOW 패널은 최근 90초 안에 heartbeat를 보낸 브라우저의 닉네임과 현재 랭킹 버전 Quick 최고 점수를 계속 표시합니다. Practice 화면은 별도로 자신의 Quick 최고 순위를 기준으로 위·아래 최대 5명의 pace도 보여 주지만, Practice 점수는 온라인 랭킹에 제출되지 않습니다.
...
개인 학습 데이터는 브라우저의 `pythonTypingSurvival:v1` 키 하나에 저장합니다. 닉네임, 설정, 최근 100개 세션, 완료한 세션의 분당 타수 추세 2,000개, 개인 최고 기록, 문법별 최근 20회 결과를 포함합니다. Progress에서 분당 타수 추세 그래프를 볼 수 있습니다. Settings에서 무엇이 삭제되는지 확인한 뒤 전체 초기화할 수 있습니다.
