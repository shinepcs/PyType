...
Beginner Guide의 가로/세로 버튼으로 원문·입력 영역을 좌우 또는 상하로 바꿀 수 있습니다. 세로 모드는 양쪽을 3줄 높이로 표시하고 현재 입력 줄을 가운데에 두도록 함께 스크롤합니다. 선택한 레이아웃은 이 브라우저에 저장됩니다.

오타와 목표 시간을 넘긴 문제는 같은 세션에서 3~7문제 뒤 다시 등장합니다. 정확도, WPM, 실시간 초당 타수, 콤보, 점수, 위험도, 문법별 숙련도는 `PRD.md`의 공식으로 계산됩니다. 제한시간은 온라인 랭크 대상인 Quick Play에만 적용됩니다.
